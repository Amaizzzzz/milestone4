...
public class WeaponStatsBonusDao {
    private static WeaponStatsBonusDao instance = null;
    private final WeaponDao weaponDao = WeaponDao.getInstance();
    private final StatisticDao statisticDao = StatisticDao.getInstance();
    
    protected WeaponStatsBonusDao() {}
    
    public static WeaponStatsBonusDao getInstance() {
        if (instance == null) {
            instance = new WeaponStatsBonusDao();
        }
        return instance;
    }

    public WeaponStatsBonus create(Connection cxn, Weapon weapon,
            Statistic stats, int bonusValue) throws SQLException {
        String insertBonus = "INSERT INTO WeaponStatsBonus (itemID, statID, bonusValue) " +
                           "VALUES (?, ?, ?)";
    ...
    public WeaponStatsBonus getWeaponStatsBonusByItemIdAndStatId(
            Connection cxn, Weapon weapon, Statistic stats) throws SQLException {
        String selectBonus = "SELECT * FROM WeaponStatsBonus " +
    ...
    public List<WeaponStatsBonus> getWeaponStatsBonusByBonusValue(
            Connection cxn,
